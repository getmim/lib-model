...
class Model
{
    ...
    static function objectSwitch(array $values, string $field, array $objects, object $format, $options): array
    {
        ...
        foreach ($map_values as $type => &$vals) {
            ...
            if (!isset($cases->{$type})) {
                $vals->result = self::asId($vals->values);
                continue;
            }
            $case = $cases->{$type};
            ...
            if (is_null($options)) {
                ...
                if(isset($case->model->type)){
                    foreach($vals->result as $index => $val){
                        $val->id = Formatter::typeApply($case->model->type, $val->id, 'id', $val, (object)[], null);
                        $vals->result[$index] = $val;
                    }
                }
            } else {
